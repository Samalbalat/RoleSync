 ...
 * It allows users to create, retrieve, and update their profiles based on their
 * role type (Tabletop or Written).
 * The profile data includes a profile name, an image URL, and a description.
 * The controller uses JWT authentication to ensure that only authenticated
 * users can access and modify their profiles.
 */
@RestController
...
public class ProfileController {
    ...
    /**
     * Get the profile of the authenticated user with the given roleType. If the
     * profile does not exist, return a 404 Not Found response.
     * 
     * @param authentication The authentication object containing the user's details
     * @param roleType       The role type of the profile to be retrieved
     * @return ResponseEntity containing the profile data if found, or a 404 Not
     *         Found response if the profile does not exist
     */
    ...
    public ResponseEntity<?> getProfile(Authentication authentication,
        ...
        if (profile.isPresent()) {
            ProfileOutDTO response = profile.map(p -> new ProfileOutDTO(p.getId(), p.getProfilename(), p.getImage(),
                    p.getDescription(), reviewService.getSummary(ReviewTargetType.PROFILE, p.getId()))).orElse(null);
    ...
    /**
     * Update the profile of the authenticated user with the given roleType. If the
     * profile does not exist, return a 404 Not Found response.
     * 
     * @param authentication The authentication object containing the user's details
    ...
    public ResponseEntity<?> putProfile(Authentication authentication,
        ...
        if (profileRepository.findByProfilename(profilePutInDTO.getProfileName()).isPresent()
                && !activeProfile.getProfilename().equals(profileName)) {
    ...
    /**
     * Create a profile for the authenticated user with the given roleType if it
     * does not exist already.
     * If a profile with the same roleType already exists for the user, return a 403
     * Forbidden response.
     *
    ...
    public ResponseEntity<?> postProfile(Authentication authentication,
        ...
        } else if (userProfiles.stream().anyMatch(p -> p.getProfileType().toString().equalsIgnoreCase(roleType)
                || p.getProfilename().equals(profilePostInDTO.getProfileName()))) {
            return ResponseEntity.status(403)
                    .body("User already has a profile for this role type or profile name is already taken");
        }
